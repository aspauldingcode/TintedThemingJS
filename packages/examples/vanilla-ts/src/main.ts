...
async function init() {
    ...
    // State
    let availableThemes: (Base16Theme | Base24Theme)[] = [defaultDark, defaultLight]
    let remoteThemePaths: string[] = []
    ...
    function renderApp() {
        ...
        const components: Record<string, ComponentRenderer> = {
            ...
            'theme-switcher': () => {
                ...
                availableThemes.forEach(t => {
                    const opt = document.createElement('option')
                    const themeId = `${(t as any).system || 'base16'}-${t.name}`
                    opt.value = themeId
                    opt.textContent = t.name

                    const currentId = `${(themeManager.getTheme() as any).system || 'base16'}-${themeManager.getTheme().name}`
                ...
                if (remoteThemePaths.length > 0) {
                    ...
                    const existingIds = new Set(availableThemes.map(t => `${(t as any).system || 'base16'}-${t.name}`))
                ...
                select.addEventListener('change', async (e: Event) => {
                    ...
                    const alreadyLoaded = availableThemes.find(t => `${(t as any).system || 'base16'}-${t.name}` === val)
                    ...
                    if (alreadyLoaded) {
                        ...
                        try {
                            ...
                            target.options[target.selectedIndex].text = 'Loading...'
        ...
        const renderer = new BlueprintDomRenderer(layoutDef, components, 'Vanilla TS')
        ...
        if (main) {
            ...
            renderMainContent(container as HTMLElement, themeManager.getTheme())
        }
    }
    ...
    Promise.all(popular.map(p => loader.loadTheme(p).catch(() => null))).then(loaded => {
        const valid = loaded.filter((t): t is Base16Theme | Base24Theme => t !== null)
        if (valid.length > 0) {
...
function renderMainContent(container: HTMLElement | null, theme: Base16Theme | Base24Theme) {
    if (!container) return
    container.innerHTML = ''
    ...
    // Safe cast or optional chaining for system
    const systemName = (theme as any).system || 'base16'
    ...
    paletteEntries.forEach(([key, color]) => {
        ...
        card.addEventListener('click', () => navigator.clipboard.writeText(color))
    ...
    let semanticsToRender: SemanticColors | Record<string, string> = {}
